...
import {
  Button, FAB, IconButton, useTheme, TextInput, HelperText,
...
export function CustomIconButton({
  onPress, size = iconSize, color = primaryColor, name, styleP,
}) {
  const { colors } = useTheme();
  const style = styles(colors);
...
export function CustomFab({
  title, onPress, color, icon, small, visible, bottomRight, loading,
}) {
  const { colors } = useTheme();
  const style = styles(colors);
